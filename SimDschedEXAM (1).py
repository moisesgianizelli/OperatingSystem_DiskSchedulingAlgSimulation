...
def outputResults(start, toDo, tracksCrossed, algType, requests):
	...
	print("Track requests were processed by ", algType,"in the following order: ",snum, group, dt, "\n", toDo)
	...
	print("\nTotal number of tracks crossed:",sum, snum, group, dt,"\n")			#total seek distance as measure of efficiency
...
def showData(pending, start, top, bottom):
	...
	print("\nAssuming the disk tracks are numbered from",bottom,"to",top)
	print("and the disk arm for",snum, group, dt,"is initially at track",start)
...
def FIFO(requests,start, top, bottom):
	...
	while len(toDo) > 0:			#while requests to do
		...
		if addData=="y" or addData=="Y":
			...
			if newData == "y" or newData=="Y":						#if so gather new data 
...
top=799					#max disk track number - This can be changed to uniquely identify a particular simulator in the output

group=""
while (not group.isalnum()):
    group=removePunc(my_input("Please enter your class group (e.g. SDH2C): "))
snum=""
while (not snum.isalnum()):
    snum=removePunc(my_input("Please enter your student ID (e.g. R00012345 with no spaces or punctuation): "))
...
print("\n\n\n\n\t\t\tThank you ", snum, " of ", group, dt)
print("\t\t\tPlease include a snip of this ID number, group name and timestamp in your submitted document.\n")
print("\t\t\tAnd, when they appear, also please include later snips of the ID number, group name and timestamp in your submitted document.\n\n\n\n\n")
...
while ans!=0:			#keep looking for a valid choice
	...
	if (len(trackNums)>0):	#if there is data
		...
		maxOption=1			#just the data generation menu
	...
	if ans!=0 and (len(trackNums)>0):						#if there is data
